...
timeoutCompile()
{
  ...
  outfile=$2
  if [ $DASHO ]; then
    nice $EXE -1 -o $outfile.output1 $infile 2> $outfile.error1 &
    PID=$!
  else
    nice $EXE -1 $infile 1> $outfile.output1 2> $outfile.error1 &
    PID=$!
  fi
...
cleanup()
{
  touch $STUB.output1 $STUB.error1 $STUB
  rm $STUB.output1 $STUB.error1 $STUB
}
...
# check for options/switches
  ...
  EXOUT="$arg.extra.output1"
  EXERR="$arg.extra.error1"
  BAOUT="$arg.output1"
  BAERR="$arg.error1"
...
# are we generating outputs?
  ...
  if [ $GENBASIC ]; then
    echo generating regular outputs for $arg in mode 1
    $EXE -1 -o $BAOUT $arg 2> $BAERR
  ...
  if [ $GENEXTRA ]; then
    echo generating extra credit outputs for $arg in mode 1
    $EXE -1 -o $EXOUT $arg 2> $EXERR
...
#
  if [ -f $BAERR ]; then
    STAT_BA=`compareInvalid $BAERR $STUB.error1`
  else
    STAT_BA=`compareValid $BAOUT $STUB.output1 $STUB.error1`
  fi

  if [ -f $EXERR ]; then
    STAT_EX=`compareInvalid $EXERR $STUB.error1`
  else
    STAT_EX=`compareValid $EXOUT $STUB.output1 $STUB.error1`
  fi

  if [ $SUMMARY ]; then
    summaryLine $arg $STAT_BA $STAT_EX
  else
    detailLine $STAT_BA "Basic" $BAOUT $BAERR $STUB.output1 $STUB.error1
    detailLine $STAT_EX "Extra Credit" $EXOUT $EXERR $STUB.output1 $STUB.error1
  fi

  rm $STUB.output1 $STUB.error1
done
cleanup
